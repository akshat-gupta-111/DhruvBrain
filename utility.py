...
CHANNELS = 6                   # ReSpeaker 4-Mic Array driver channel map
...
def main():
    ...
    try:
        # Directly target the working Pulse node (Index 26) to bypass ALSA channel warnings
        with sd.InputStream(device=INPUT_DEVICE_INDEX, channels=CHANNELS,
                             samplerate=SAMPLE_RATE, dtype='int16') as stream:
    ...
    mono_audio = full_audio[:, 0]  # Isolate microphone channel 0
